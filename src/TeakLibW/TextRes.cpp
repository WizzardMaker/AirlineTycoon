...
const char* ExcTextResStaticOverflow    = "TextRes is too int: %lx:%lx";
...
void TEXTRES::Open(char const* source, void* cached)
{
    ...
    if (cached)
    {
        ...
        int AnzStrings = 0;
        int AnzEntries = 0;
        for (int i = 0, j = 0; i < FileBuffer.AnzEntries(); i += j)
        {
        ...
        for (int i = 0; i < Entries.AnzEntries(); ++i)
...
char* TEXTRES::GetP(unsigned int group, unsigned int id)
{
    char* text = NULL;
    for (int i = 0; i < Entries.AnzEntries(); ++i)
    {
...
char* TEXTRES::GetS(unsigned int group, unsigned int id)
{
