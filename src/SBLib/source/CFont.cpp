...
int SB_CFont::DrawTextBlock(class SB_CBitmapCore* bmp, RECT* block, char* str, int length, int offset, bool hidden)
{
    this->Bitmap = bmp;
    this->Hidden = hidden;
    if (length == 0) {
        ...
        while (length != 0) {
            ...
            if (i > 0) {
                if (block->right < this->Pos.x + GetWidth(str,i)) {
                    this->Pos.y = this->Pos.y + int(this->Header.Height * LineSpace);
                    this->Pos.x = block->left;
                }
...
void SB_CFont::DrawTextWithTabs(class SB_CBitmapCore* bmp, int x, int y, char* str, int length, bool hidden)
{
    int i = 0;
    this->Bitmap = bmp;
    this->Hidden = hidden;
...
void SB_CFont::SetTabulator(TABS* pTabs, unsigned int szTabs)
{
    if (this->Tabulator) {
...
int SB_CFont::PreviewTextBlock(class SB_CBitmapCore * bmp, RECT * block, char* str, int length, int offset, bool hidden)
{
    this->Bitmap = bmp;
    this->Hidden = hidden;
    if (length == 0) {
...
int SB_CFont::DrawTextBlock(class SB_CBitmapCore* bmp, int l, int t, int r, int b, char* str, int length, int offset, bool hidden)
{
   RECT rect{ l, t, r, b };
   return DrawTextBlock(bmp, &rect, str, length, offset, hidden);
}

int SB_CFont::PreviewTextBlock(class SB_CBitmapCore* bmp, int l, int t, int r, int b, char* str, int length, int offset, bool hidden)
{
   RECT rect{ l, t, r, b };
   return PreviewTextBlock(bmp, &rect, str, length, offset, hidden);
}

int SB_CFont::GetWidthAt(char* str, int offset, char ch)
{
    int i = 0;
    while( true ) {
        switch(*str) {
...
int SB_CFont::GetWordLength(char* str, int offset)
{
    dword length = 0;
    while (offset != 0) {
        switch(str[length]) {
...
int SB_CFont::GetWidth(char* str, int offset)
{
    int width = 0;
    if (offset == 0)
    {
...
bool SB_CFont::DrawWord(char* str, int offset)
{
    do {
        if (offset == 0) {
            return true;
